...
Tất cả những thay đổi đáng chú ý sẽ được thêm vào tài liệu này.

Định dạng văn bản dựa trên [Keep a Changelog](https://keepachangelog.com/en/1.0.0/) và dự án này tuân thủ [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

- `Added` cho các tính năng mới nhất.
- `Changed` cho các chỉnh sửa của các tính năng sẵn có.
- `Deprecated` cho các tính năng mà sắp bị gỡ bỏ ở các phiên bản tiếp theo.
- `Removed` các tính năng đã được gỡ bỏ.
- `Fixed` cho các lỗi hoặc bug đã được sửa.
